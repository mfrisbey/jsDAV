...
var jsDAV_Common = require("./common");
...
(function() {
    ...
    this.handleError = function(e) {
        ...
        xml += '<a:jsdav-version>' + jsDAV_Common.getVersion() + '</a:jsdav-version>\n';
    ...
    this.httpOptions = function() {
        ...
        this.getAllowedMethods(uri, function(err, methods) {
            ...
            var headers = {
                ...
                "MS-Author-Via"   : "DAV",
                "Accept-Ranges"   : "bytes",
                "X-jsDAV-Version" : jsDAV_Common.getVersion(),
                "Content-Length"  : 0
            };
    ...
    this.createCollection = function(uri, resourceType, properties, cbcreatecoll) {
        ...
        function checkParent(cbcheckparent) {
          self.getNodeForPath(parentUri, function (err, parent) {
            if (!Util.empty(err)) {
              self.createCollection(parentUri, resourceType, properties, function (err) {
                if (err) {
                  cbcheckparent(err);
                } else {
                  self.getNodeForPath(parentUri, function (err, parent) {
                    cbcheckparent(err, parent);
                  });
                }
              });
            } else {
              cbcheckparent(null, parent);
            }
          });
        }

        // Making sure the parent exists
        checkParent(function(err, parent) {
            if (!Util.empty(err))
